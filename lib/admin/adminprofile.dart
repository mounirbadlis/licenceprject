...
class AdminProfile extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => APPages();
}

class APPages extends State<AdminProfile> {
  bool visible = false;
  ...
  void showEditProfileDialog() {
    ...
    phone.text = Provider.of<Auth>(context, listen: false).user!["phone"]; adresse.text = Provider.of<Auth>(context, listen: false).user!["adresse"];
    showDialog(
      context: context,
                        ...
                        TextButton(
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                                http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/admin.php"), body: { "method" : "editprofile", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "nom" : nom.text, "prenom" : prenom.text, "phone" : phone.text, "adresse" : adresse.text});
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              if(email.text == Provider.of<Auth>(context, listen: false).user!["email"]){
                                ...
                                http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/admin.php"), body: { "method" : "changeemail", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "email" : email.text,});
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              if(currentpassword.text == Provider.of<Auth>(context, listen: false).user!["password"]){
                                http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/admin.php"), body: { "method" : "changepassword", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "password" : password.text,});
                                var data = jsonDecode(response.body);
                                Provider.of<Auth>(context, listen: false).user = data[0];
                                Navigator.push(context, MaterialPageRoute(builder: (builder) => AdminProfile()));
                                Fluttertoast.showToast(
