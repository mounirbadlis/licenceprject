...
class DoctorProfile extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => DPPages();
}

class DPPages extends State<DoctorProfile> {
  bool visible = false;
          ...
          child: Column(
            children: [
              ...
              Container( width: double.infinity, height: MediaQuery.of(context).size.height / 14, padding: EdgeInsets.all(MediaQuery.of(context).size.height * 0.004),
                child: Row(
                  children: [
                    Icon(Icons.medical_services, color: Provider.of<Auth>(context, listen: false).secondarycolor, size: MediaQuery.of(context).size.height * 0.05,),
                    SizedBox(width: MediaQuery.of(context).size.width / 30,),
                    Text("${Provider.of<Auth>(context, listen: false).user!["specialite"]}", style: TextStyle(color: Provider.of<Auth>(context, listen: false).secondarycolor, fontSize: MediaQuery.of(context).size.width *0.04),),
                  ],
                ),
              ),
  ...
  void showEditProfileDialog() {
    ...
    phone.text = "0" + Provider.of<Auth>(context, listen: false).user!["phone"]; adresse.text = Provider.of<Auth>(context, listen: false).user!["adresse"]; specialete.text = Provider.of<Auth>(context, listen: false).user!["specialite"];
    showDialog(
      context: context,
                        ...
                        TextButton(
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/login.php"), body: { "method" : "editprofile", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "changes" : jsonEncode({"nom" : nom.text, "prenom" : prenom.text, "phone" : phone.text, "adresse" : adresse.text, "specialite" : specialete.text}),});
                              Navigator.push(context, MaterialPageRoute(builder: (builder) => DoctorProfile()));
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              if(email.text == Provider.of<Auth>(context, listen: false).user!["email"]){
                                ...
                                http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/doctor.php"), body: { "method" : "changeemail", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "email" : email.text,});
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              if(currentpassword.text == Provider.of<Auth>(context, listen: false).user!["password"]){
                                http.Response response = await http.post(Uri.parse("http://${Provider.of<Auth>(context, listen: false).baseurl}/mydb/doctor.php"), body: { "method" : "changepassword", "id_utilisateur" : Provider.of<Auth>(context, listen: false).user!["id_utilisateur"], "password" : password.text,});
                                var data = jsonDecode(response.body);
                                Provider.of<Auth>(context, listen: false).user = data[0];
                                Navigator.push(context, MaterialPageRoute(builder: (builder) => DoctorProfile()));
                                Fluttertoast.showToast(
